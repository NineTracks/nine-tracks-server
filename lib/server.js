...
function cleanupChannelFromFirebase(peerId) {
  console.log("Cleaning up " + peerId + "...");
  admin.firestore().collection("channels").where("peerId", "==", peerId).get().then(function(query) {
    if (!query.empty) {
      query.docs[0].ref.delete().catch(console.error);
    }
  }).catch(console.err);
}
...
app._removePeer = function(key, id) {
  if (this._clients[key] && this._clients[key][id]) {
    ...
    // Remove channel from Firebase after peer is removed
    cleanupChannelFromFirebase(id);
